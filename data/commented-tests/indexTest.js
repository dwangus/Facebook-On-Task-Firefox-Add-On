...
pageMod.PageMod({
  ...
  //contentScriptOptions: {"message": "Hello World!"},//An example
...
function sending(worker) {
	...
	/*worker.port.on('test', function(url) {
		worker.port.emit('arrived2', url);
	});*/
}
...
panel.port.on("openChat", function(chatSearch) {
	...
	var chat = tabs.activeTab.attach({
		contentScriptFile: [data.url("jquery-3.1.0.min.js"), data.url("emulatetab.joelpurra.min.js"), data.url("chat.js")]
	});
